...
Live MVP thin-slice deployment (us-east-1): web at the `WebUrl` stack output (CloudFront), API at `ApiUrl`. The API is protected by a Cognito JWT authorizer; unauthenticated requests return 401. Auth uses the Cognito Hosted UI. Infra is cost-tuned: DynamoDB on-demand, CloudFront PRICE_CLASS_100, Lambda 256 MB, API Gateway stage throttling (20 req/s, burst 40). `apps/web/.env.production` is generated by `deploy-web.sh` from stack outputs and is gitignored. The public marketing site is `apps/site` (Astro, static) served at `/`, `/uk/`, `/privacy`, `/terms` from the same bucket/distribution; the SPA's home is `/garage` and its shell ships as `app.html`. A CloudFront viewer-request Function routes by URI: app routes (the shared list in `packages/config/app-routes.ts`, also used by the service worker allowlist and the generated `robots.txt`) → `app.html`; `/`, `/uk/` and extensionless site pages → `<dir>/index.html`; anything else untouched. Missing S3 keys (403/404) return a real HTTP 404 with the site's `404.html` — the shell never answers for unknown URLs, so crawlers do not see soft-404s. `deploy-web.sh` merges both builds.
